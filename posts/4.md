...
Finally, `rb_mod_module_eval` is called. This is the C function implementing `Module#module_eval`. Because this function is called directly, bypassing the Ruby call machinery, the block passed to `Module#refine` is also accessible by `Module#module_eval` (this is because blocks are passed in the current `rb_control_frame_t`, not via the C stack).

We talked about Ruby's variadic method calling convention earlier, and here's an example of it! Since no arguments are passed to `Module#module_eval`, it is safe to pass a null pointer as `argv`.
...
This method returns a **copy** of the internal `__refinements__` hash, or an empty hash if `__refinements__` is not set.
...
In pseudo-Ruby, those two functions would look something like this:

    \ruby
    class Module
      def refinements
        if @__refinements__.nil?
          return {}
        end
        result = {}
        @__refinements__.each do |k, v|
          result[k] = v
        end
        result
      end
...
We're getting into the meat of it now! `Module#using` only takes a single module as its argument - the module which refinements from should be used - although a scope can use refinements from multiple modules.

Refinements are both lexically and dynamically scoped and we'll take a look at the C implementation of the `#using` method to see exactly how it does this:
